...
rm -rf /usr/local/sbin/kaizensystem_exp
wget -q -O /usr/local/sbin/kaizensystem_exp "https://raw.githubusercontent.com/rewasu91/scvps/main/Setup/kaizensystem_exp"
chmod 755 /usr/local/sbin/kaizensystem_exp
...
function startStopWarpProxy() {
	...
	if [[ -z $(which warp-svc) && $(systemctl is-active warp-svc) == "inactive" ]]; then
		...
		if [[ ${INSTALL_WARP} == "y" ]]; then
			...
			wget -q -O /etc/kaizensystem/warp_outbounds.txt "https://raw.githubusercontent.com/rewasu91/scvps/main/Setup/warp_outbounds.txt"
			if [[ $? != 0 ]]; then
		...
		if [[ ${ENABLE_WARP} == "y" ]]; then
			...
			if [[ ! -e /etc/kaizensystem/warp_outbounds.txt ]]; then
				wget -q -O /etc/kaizensystem/warp_outbounds.txt "https://raw.githubusercontent.com/rewasu91/scvps/main/Setup/warp_outbounds.txt"
				if [[ $? != 0 ]]; then
					echo -e " ${RED}An error occured. Please try again${NOCOLOR}"
					echo
			...
			grep -A100 "outbounds" /usr/local/etc/xray/Admin.json | tee /etc/kaizensystem/outbounds.txt
			restartWARP
			sleep 2
			echo -e "${GREEN}done${NOCOLOR}"
			echo
		...
		if [[ ${DISABLE_WARP} == "y" ]]; then
			...
			if [[ ! -e /etc/kaizensystem/outbounds.txt ]]; then
				wget -q -O /etc/kaizensystem/outbounds.txt "https://raw.githubusercontent.com/rewasu91/scvps/main/Setup/warp_outbounds.txt"
				if [[ $? != 0 ]]; then
					echo -e " ${RED}An error occured. Please try again${NOCOLOR}"
					echo
					read -p " Press Enter to continue..."
			...
			grep -A100 "outbounds" /usr/local/etc/xray/Admin.json | tee /etc/kaizensystem/warp_outbounds.txt
			restartWARP
			echo -e "${GREEN}done${NOCOLOR}"
			echo
			read -p " Press Enter to continue..."
...
function stopWARPProxy() {
	...
	if [[ -z $(which warp-cli) && $(systemctl is-active warp-svc) == "inactive" ]]; then
		...
		if [[ ${STOP_WARP,,} == "y" ]]; then
			...
			systemctl disable warp-svc >/dev/null 2>&1
			if [[ ! -e /etc/kaizensystem/outbounds.txt ]]; then
				wget -q -O /etc/kaizensystem/outbounds.txt "https://raw.githubusercontent.com/rewasu91/scvps/main/Setup/outbounds.txt"
				if [[ $? != 0 ]]; then
					echo -e " ${RED}An error occured${NOCOLOR}"
					echo
					read -p " Press Enter to continue..."
...
function restartWARPProxy() {
	...
	if [[ ! -z $(which warp-cli) && $(systemctl is-active warp-svc) == "active" ]]; then
		...
		source /etc/kaizensystem/warp_trace.txt
...
function enableAdvancedWARPProxy() {
	...
	if [[ ! -z $(which warp-cli) && $(systemctl is-active warp-svc) == "active" && $(grep -c -w "\"network\": \"udp,tcp\"" /usr/local/etc/xray/config.json) == 0 ]]; then
		...
		if [[ ${ENABLE_ADVANCED_WARP,,} == "y" ]]; then
			...
			if [[ ! -e /etc/kaizensystem/full_warp_outbounds.txt ]]; then
				wget -q -O /etc/kaizensystem/full_warp_outbounds.txt "https://raw.githubusercontent.com/rewasu91/scvps/main/Setup/full_warp_outbounds.txt"
				if [[ $? != 0 ]]; then
					echo -e " ${RED}An error occured. Please try again${NOCOLOR}"
					echo
					read -p " Press Enter to continue..."
...
function addRemoveDomain() {
	...
	echo
	until [[ ! -z ${DOMAIN} && ${DOMAIN,,} != "kaizenvpsdomain.com" ]]; do
	...
	if [[ $(grep -c -w "\"network\": \"udp,tcp\"" /usr/local/etc/xray/config.json) == 0 && $(grep -c "${DOMAIN}" /usr/local/etc/xray/config.json) != 0 && $(grep "${DOMAIN}" /usr/local/etc/xray/config.json) != "kaizenvpsdomain.com" ]]; then
		clear
		echo
...
function WARPProxyStatus() {
	if [[ ! -e /etc/kaizensystem/warp_trace.txt ]]; then
		curl -x http://127.0.0.1:40000 -sL https://cloudflare.com/cdn-cgi/trace >/etc/kaizensystem/warp_trace.txt
	fi
	WARP_ACTIVE=$(systemctl status warp-svc.service | sed -ne 's|.*Active: active.*-.* \(.*\) +.*$|\1|p')
	WARP_SECONDS=$(date -d "${WARP_ACTIVE}" +%s)
	TRACE_TIME=$(stat -c %x /etc/kaizensystem/warp_trace.txt | sed -ne 's|.*-.* \(.*\)\..* +.*$|\1|p')
	TRACE_SECONDS=$(date -d "${TRACE_TIME}" +%s)
	COUNT=$((TRACE_SECONDS-WARP_SECONDS))
	if [[ $(echo ${COUNT}) -le 0 ]]; then
		curl -x http://127.0.0.1:40000 -sL https://cloudflare.com/cdn-cgi/trace >/etc/kaizensystem/warp_trace.txt
	fi
	IP=$(grep "ip" /etc/kaizensystem/warp_trace.txt | cut -d'=' -f2)
	COLO=$(grep "colo" /etc/kaizensystem/warp_trace.txt | cut -d'=' -f2)
	LOC=$(grep "loc" /etc/kaizensystem/warp_trace.txt | cut -d'=' -f2)
	TYPE=$(grep "warp" /etc/kaizensystem/warp_trace.txt | cut -d'=' -f2)
	...
	if [[ $(systemctl is-active warp-svc) == "active" ]]; then
		...
		if [[ $(grep -c -w "\"network\": \"udp,tcp\"" /usr/local/etc/xray/config.json) == 0 && ${TYPE} == "on" ]]; then
			printf " %-41s %1s ${GREEN}%-7s${NOCOLOR}" "WARP Proxy" ":" "running"
			...
			printf " %-41s %1s ${GREEN}%-7s${NOCOLOR}" "Advanced WARP Proxy" ":" "running"
		elif [[ $(grep -c -w "\"network\": \"udp,tcp\"" /usr/local/etc/xray/config.json) != 0 && ${TYPE} == "plus" ]]; then
			printf " %-41s %1s ${GREEN}%-7s${NOCOLOR}" "Advanced WARP+ Proxy" ":" "running"
		fi
		printf "\n %-41s %1s ${GREEN}%-7s${NOCOLOR}" "Location" ":" "${COLO} (${LOC})"
		printf "\n %-41s %1s ${GREEN}%-7s${NOCOLOR}" "Active since" ":" "${ACTIVE_SINCE}"
		printf "\n %-41s %1s ${GREEN}%-7s${NOCOLOR}" "IP Address" ":" "${IP}"
	fi
}
...
# 			#rm -rf /etc/kaizensystem/iptv/${PROVIDER_NAME}.playlist
# 			#sleep 2
# 			#echo -n -e "done"
# 			#sleep 1
# 			#registerProvider
...
# 		registerProvider
# 	fi
# 	until [[ ! -z ${URL} && $(grep -c -w "${URL}" /etc/kaizensystem/iptv/providerlist.txt) == 0 ]]; do
...
# 			#sed -i "/^${PROXY_PASSWORD}/d" /etc/kaizensystem/parameter
# 			#sed -i "/^${PROXY_USER}/d" /etc/kaizensystem/parameter
# 			#sed -i "/^${PROXY_IP}/d" /etc/kaizensystem/parameter
# 			#sed -i "/^${PROXY_PORT}/d" /etc/kaizensystem/parameter

# 			#echo -e "PROXY_PASSWORD=${PROXY_PASSWORD}
# #PROXY_USER=${PROXY_USER}
# #PROXY_IP=${PROXY_IP}
# #PROXY_PORT=${PROXY_PORT}">>/etc/kaizensystem/parameter
# 		else
...
# 	curl -fsSL -o /etc/kaizensystem/iptv/${PROVIDER_NAME}.playlist.tmp http://${PROXY_IP}/iptv/playlist.tmp
...
# 	echo "${PROVIDER_NAME} ${URL} ${OTT_HASH}">>/etc/kaizensystem/iptv/providerlist.txt
...
<a href=\"https://shorturl.at/pvPWX\"><b>🔗 Download di sini</b></a>

<b>⚙️ CARA GUNA OTT NAVIGATOR </b>
...
echo -e "${BlueCyan}═══════════════════════════════════════════════════${plain}";
echo -e ""
read -p "  Please enter your option [1-7 atau x] :  " MENU_OPTION
echo -e ""
	case ${MENU_OPTION} in
	1)
	 	startWARPProxy
...
function onoff() {
	...
	if [ "${switch}" == "on" ]; then
		...
		echo "0 1 * * * root autobackup" >>/etc/crontab
		service cron restart >/dev/null 2>&1
		service cron reload >/dev/null 2>&1
		echo "Turn On"
	fi
...
function gorestore() {
	...
	cp -r htmliptv /var/www/html &>/dev/null
