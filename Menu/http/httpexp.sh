...
# // Start
grep -c -E "^Http " "/etc/xray-mini/client.conf" > /etc/kaizenvpn/jumlah-akun-http.db;
grep "^Http " "/etc/xray-mini/client.conf" | awk '{print $2}'  > /etc/kaizenvpn/akun-http.db;
totalaccounts=`cat /etc/kaizenvpn/akun-http.db | wc -l`;
echo "Total Akun = $totalaccounts" > /etc/kaizenvpn/total-akun-http.db;
for((i=1; i<=$totalaccounts; i++ ));
do
    # // Username Interval Counting
    username=$( head -n $i /etc/kaizenvpn/akun-http.db | tail -n 1 );
...
if [[ $sisa_hari -lt 1 ]]; then
    ...
    cat /etc/kaizenvpn/xray-mini-utils/http-backup.json | jq 'del(.inbounds[0].settings.accounts[] | select(.user == "'${username}'"))' > /etc/xray-mini/http.json;
    rm -rf /etc/kaizenvpn/http/${username};
    ...
    echo -e "";
    echo -e "${CYAN}════════════════════════════════════════════${NC}";
    echo -e "${WBBG}       [ Memadam Akaun Expire HTTP ]        ${NC}";
    echo -e "${CYAN}════════════════════════════════════════════${NC}";
    echo -e "";
    echo "  Username : $username | Expire Pada : $expired | Dipadam Pada : $now" >> /etc/kaizenvpn/http-expired-deleted.db;
