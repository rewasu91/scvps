...
grep -c -E "^#DEPAN " "/etc/wireguard/wg0.conf" > /etc/kaizenvpn/jumlah-akun-wireguard.db;
grep "^#DEPAN " "/etc/wireguard/wg0.conf" | cut -d ' ' -f 4  > /etc/kaizenvpn/akun-wireguard.db;
totalaccounts=`cat /etc/kaizenvpn/akun-wireguard.db | wc -l`;
echo "Total Akun = $totalaccounts" > /etc/kaizenvpn/total-akun-wireguard.db;
for((i=1; i<=$totalaccounts; i++ ));
do
    # // Username Interval Counting
    username=`head -n $i /etc/kaizenvpn/akun-wireguard.db | tail -n 1`;
...
if [[ $sisa_hari -lt 1 ]]; then
    ...
    rm -f /etc/kaizenvpn/webserver/wg-client/$client.conf;
    rm -rf /etc/kaizenvpn/wireguard/$client;
    ...
    echo "";
    echo -e "${CYAN}════════════════════════════════════════════${NC}";
    echo -e "${WBBG}     [ Memadam Akaun Expire Wireguard ]     ${NC}";
    echo -e "${CYAN}════════════════════════════════════════════${NC}";
    echo -e "";
    echo "Username : $username | Expire Pada : $expired | Dipadam Pada : $now" >> /etc/kaizenvpn/wireguard-expired-deleted.db;
