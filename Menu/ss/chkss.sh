...
echo -n > /etc/kaizenvpn/cache/shadowsocks_temp.txt;
echo -n > /etc/kaizenvpn/cache/shadowsocks_temp2.txt;
data=(`cat /etc/xray-mini/client.conf | grep '^Shadowsocks' | cut -d " " -f 2`);
echo "";
echo -e "${CYAN}════════════════════════════════════════════${NC}";
echo -e "${WBBG}    [ Senarai Login Akaun Shadowsocks ]     ${NC}";
echo -e "${CYAN}════════════════════════════════════════════${NC}";
echo -e "";
for akun in "${data[@]}"
do
...
for ip in "${data2[@]}"
do
jum=$(cat /etc/kaizenvpn/xray-mini-shadowsocks/access.log | grep -w $akun | awk '{print $3}' | cut -d: -f1 | grep -w $ip | sort | uniq);
if [[ "$jum" = "$ip" ]]; then
echo "$jum" >> /etc/kaizenvpn/cache/shadowsocks_temp.txt;
else
echo "$ip" >> /etc/kaizenvpn/cache/shadowsocks_temp2.txt;
fi
jum2=$(cat /etc/kaizenvpn/cache/shadowsocks_temp.txt);
sed -i "/$jum2/d" /etc/kaizenvpn/cache/shadowsocks_temp2.txt > /dev/null 2>&1;
done
jum=$(cat /etc/kaizenvpn/cache/shadowsocks_temp.txt);
if [[ -z "$jum" ]]; then
echo > /dev/null;
else
jum2=$(cat /etc/kaizenvpn/cache/shadowsocks_temp.txt | nl);
hariini=`date -d "0 days" +"%Y-%m-%d"`;
echo "Username : $akun";
echo "$jum2";
echo "";
fi
rm -rf /etc/kaizenvpn/cache/shadowsocks_temp.txt;
done
echo -e "${CYAN}════════════════════════════════════════════${NC}";
echo "";
rm -rf /etc/kaizenvpn/cache/shadowsocks_temp2.txt;
