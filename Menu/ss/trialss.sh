...
echo "";
echo -e "${CYAN}════════════════════════════════════════════${NC}";
echo -e "${WBBG}  [ Membuat Akaun Percubaan Shadowsocks ]   ${NC}";
echo -e "${CYAN}════════════════════════════════════════════${NC}";
echo -e "";
...
# // Getting Shadowsocks port using grep from config
...
cat /etc/kaizenvpn/xray-mini-utils/shadowsocks-backup.json | jq '.inbounds[0].settings.clients += [{"method": "chacha20-ietf-poly1305","password": "'${Username}'","email": "'${Username}'" }]' > /etc/xray-mini/shadowsocks.json;
...
mkdir -p /etc/kaizenvpn/shadowsocks/;
mkdir -p /etc/kaizenvpn/shadowsocks/${Username};
rm -f /etc/kaizenvpn/shadowsocks/${Username}/config.log;

# ═══════════════════════════════════════
# // Maklumat Akaun Percubaan Shadowsocks
# ═══════════════════════════════════════
clear; 
...
echo -e "${WBBG}  [ Maklumat Akaun Percubaan Shadowsocks ]  ${NC}" | tee -a /etc/kaizenvpn/shadowsocks/${Username}/config.log;
echo -e "${CYAN}════════════════════════════════════════════${NC}" | tee -a /etc/kaizenvpn/shadowsocks/${Username}/config.log;
