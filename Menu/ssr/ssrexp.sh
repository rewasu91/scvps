...
# // Start
grep -c -E "^SSR " "/etc/kaizenvpn/ssr-client.conf" > /etc/kaizenvpn/jumlah-akun-ssr.db;
grep "^SSR " "/etc/kaizenvpn/ssr-client.conf" | awk '{print $2}'  > /etc/kaizenvpn/akun-ssr.db;
totalaccounts=`cat /etc/kaizenvpn/akun-ssr.db | wc -l`;
echo "Total Akun = $totalaccounts" > /etc/kaizenvpn/total-akun-ssr.db;
for((i=1; i<=$totalaccounts; i++ ));
do
    # // Username Interval Counting
    username=$( head -n $i /etc/kaizenvpn/akun-ssr.db | tail -n 1 );
    expired=$( grep "^SSR " "/etc/kaizenvpn/ssr-client.conf" | grep -w $username | head -n1 | awk '{print $3}' );
...
if [[ $sisa_hari -lt 1 ]]; then
    ...
    sed -i "/\b$username\b/d" /etc/kaizenvpn/ssr-client.conf;
