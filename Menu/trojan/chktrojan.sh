...
echo -n > /etc/kaizenvpn/cache/trojan_temp.txt;
echo -n > /etc/kaizenvpn/cache/trojan_temp2.txt;
data=(`cat /etc/xray-mini/client.conf | grep '^Trojan' | cut -d " " -f 2`);
echo "";
echo -e "${CYAN}════════════════════════════════════════════${NC}";
echo -e "${WBBG}      [ Senarai Login Akaun Trojan ]        ${NC}";
echo -e "${CYAN}════════════════════════════════════════════${NC}";
echo -e "";
for akun in "${data[@]}"
do
...
if [[ "$jum" = "$ip" ]]; then
echo "$jum" >> /etc/kaizenvpn/cache/trojan_temp.txt;
else
echo "$ip" >> /etc/kaizenvpn/cache/trojan_temp2.txt;
fi
jum2=$(cat /etc/kaizenvpn/cache/trojan_temp.txt);
sed -i "/$jum2/d" /etc/kaizenvpn/cache/trojan_temp2.txt > /dev/null 2>&1;
done
jum=$(cat /etc/kaizenvpn/cache/trojan_temp.txt);
if [[ -z "$jum" ]]; then
echo > /dev/null;
else
jum2=$(cat /etc/kaizenvpn/cache/trojan_temp.txt | nl);
hariini=`date -d "0 days" +"%Y-%m-%d"`;
echo "Username : $akun";
echo "$jum2";
echo ""
fi
rm -rf /etc/kaizenvpn/cache/trojan_temp.txt;
done
echo -e "${CYAN}════════════════════════════════════════════${NC}";
echo -e "${WBBG}   [ Senarai Login Akaun GRPC Trojan ]      ${NC}";
echo -e "${CYAN}════════════════════════════════════════════${NC}";
oth=$(cat /etc/kaizenvpn/cache/trojan_temp2.txt | sort | uniq | nl );
echo "GRPC IP : All User IP";
echo "$oth";
echo -e "${CYAN}════════════════════════════════════════════${NC}";
echo ""
rm -rf /etc/kaizenvpn/cache/trojan_temp2.txt;
