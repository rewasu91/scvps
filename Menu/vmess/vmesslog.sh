...
grep -e "^Vmess " "/etc/xray-mini/client.conf" | cut -d ' ' -f 2-8 | nl -s ') ';
	until [[ ${CLIENT_002} -ge 1 && ${CLIENT_002} -le ${CLIENT_001} ]]; do
		if [[ ${CLIENT_002} == '1' ]]; then
                echo -e "${CYAN}════════════════════════════════════════════${NC}";
			read -rp " Sila masukkan nombor akaun untuk melihat log akses (1-${CLIENT_001}) : " CLIENT_002;
		else
                echo -e "${CYAN}════════════════════════════════════════════${NC}";
			read -rp " Sila masukkan nombor akaun untuk melihat log akses (1-${CLIENT_001}) : " CLIENT_002;
		fi
...
cat /etc/kaizenvpn/xray-mini-nontls/access.log | grep -w $hariini | grep -w $waktu | tail -n500 | grep -w 'accepted' | grep -w $client;
cat /etc/kaizenvpn/xray-mini-tls/access.log | grep -w $hariini | grep -w $waktu | tail -n500 | grep -w 'accepted' | grep -w $client;
