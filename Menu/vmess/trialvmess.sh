...
echo -e "";
echo -e "${CYAN}════════════════════════════════════════════${NC}";
echo -e "${WBBG}    [ Membuat Akaun Percubaan Vmess ]       ${NC}";
echo -e "${CYAN}════════════════════════════════════════════${NC}";
echo -e "";
...
cat /etc/kaizenvpn/xray-mini-utils/tls-backup.json | jq '.inbounds[2].settings.clients += [{"id": "'${uuid}'","email": "'${Username}'","alterid": '"0"'}]' > /etc/kaizenvpn/xray-mini-cache.json;
cat /etc/kaizenvpn/xray-mini-cache.json | jq '.inbounds[5].settings.clients += [{"id": "'${uuid}'","email": "'${Username}'","alterid": '"0"'}]' > /etc/xray-mini/tls.json;
cp /etc/xray-mini/nontls.json /etc/kaizenvpn/xray-mini-utils/nontls-backup.json;
cat /etc/kaizenvpn/xray-mini-utils/nontls-backup.json | jq '.inbounds[0].settings.clients += [{"id": "'${uuid}'","email": "'${Username}'","alterid": '"0"'}]' > /etc/xray-mini/nontls.json;
echo -e "Vmess $Username $exp" >> /etc/xray-mini/client.conf;

cat > /etc/kaizenvpn/cache/vmess-tls-gun.tmp << END
{"add":"${domain}","aid":"0","host":"","id":"${uuid}","net":"grpc","path":"Vmess-GRPC","port":"${tls_port}","ps":"${Username}","scy":"none","sni":"","tls":"tls","type":"gun","v":"2"}
END

cat > /etc/kaizenvpn/cache/vmess-tls-ws.tmp << END
{"add":"${domain}","aid":"0","host":"","id":"${uuid}","net":"ws","path":"/vmess","port":"${tls_port}","ps":"${Username}","scy":"none","sni":"${domain}","tls":"tls","type":"","v":"2"}
END

cat > /etc/kaizenvpn/cache/vmess-nontls.tmp << END
{"add":"${domain}","aid":"0","host":"","id":"${uuid}","net":"ws","path":"/vmess","port":"${nontls_port}","ps":"${Username}","scy":"none","sni":"","tls":"","type":"","v":"2"}
END

# // Make Config Link
grpc_link="vmess://$(base64 -w 0 /etc/kaizenvpn/cache/vmess-tls-gun.tmp)";
ws_tls_link="vmess://$(base64 -w 0 /etc/kaizenvpn/cache/vmess-tls-ws.tmp)";
ws_nontls_link="vmess://$(base64 -w 0 /etc/kaizenvpn/cache/vmess-nontls.tmp)";
...
mkdir -p /etc/kaizenvpn/vmess/${Username};
rm -f /etc/kaizenvpn/vmess/${Username}/config.log;

# ═════════════════════════════════
# // Maklumat Akaun Percubaan Vmess
# ═════════════════════════════════
clear; 
echo "" | tee -a /etc/kaizenvpn/vmess/${Username}/config.log;
echo -e "${CYAN}════════════════════════════════════════════${NC}" | tee -a /etc/kaizenvpn/vmess/${Username}/config.log;
echo -e "${WBBG}     [ Maklumat Akaun Percubaan Vmess ]     ${NC}" | tee -a /etc/kaizenvpn/vmess/${Username}/config.log;
echo -e "${CYAN}════════════════════════════════════════════${NC}" | tee -a /etc/kaizenvpn/vmess/${Username}/config.log;
echo -e " Username    ► ${Username}" | tee -a /etc/kaizenvpn/vmess/${Username}/config.log;
echo -e " Dibuat Pada ► ${hariini}" | tee -a /etc/kaizenvpn/vmess/${Username}/config.log;
echo -e " Expire Pada ► ${exp}" | tee -a /etc/kaizenvpn/vmess/${Username}/config.log;
echo -e " IP          ► ${IP_NYA}" | tee -a /etc/kaizenvpn/vmess/${Username}/config.log;
echo -e " Address     ► ${domain}" | tee -a /etc/kaizenvpn/vmess/${Username}/config.log;
echo -e " Port TLS    ► ${tls_port}" | tee -a /etc/kaizenvpn/vmess/${Username}/config.log;
echo -e " Port NTLS   ► ${nontls_port}" | tee -a /etc/kaizenvpn/vmess/${Username}/config.log;
echo -e " User ID     ► ${uuid}" | tee -a /etc/kaizenvpn/vmess/${Username}/config.log;
echo -e "${CYAN}════════════════════════════════════════════${NC}" | tee -a /etc/kaizenvpn/vmess/${Username}/config.log;
echo -e " GRPC VMESS CONFIG LINK" | tee -a /etc/kaizenvpn/vmess/${Username}/config.log;
echo -e " ${grpc_link}" | tee -a /etc/kaizenvpn/vmess/${Username}/config.log;
echo -e "${CYAN}════════════════════════════════════════════${NC}" | tee -a /etc/kaizenvpn/vmess/${Username}/config.log;
echo -e " WS TLS VMESS CONFIG LINK" | tee -a /etc/kaizenvpn/vmess/${Username}/config.log;
echo -e " ${ws_tls_link}" | tee -a /etc/kaizenvpn/vmess/${Username}/config.log;
echo -e "${CYAN}════════════════════════════════════════════${NC}" | tee -a /etc/kaizenvpn/vmess/${Username}/config.log;
echo -e " WS NTLS VMESS CONFIG LINK" | tee -a /etc/kaizenvpn/vmess/${Username}/config.log;
echo -e " ${ws_nontls_link}" | tee -a /etc/kaizenvpn/vmess/${Username}/config.log;
echo -e "${CYAN}════════════════════════════════════════════${NC}" | tee -a /etc/kaizenvpn/vmess/${Username}/config.log;
