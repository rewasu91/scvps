...
echo -n > /etc/kaizenvpn/cache/vless_temp.txt;
echo -n > /etc/kaizenvpn/cache/vless_temp2.txt;
data=(`cat /etc/xray-mini/client.conf | grep '^Vless' | cut -d " " -f 2`);
echo "";
echo -e "${CYAN}════════════════════════════════════════════${NC}";
echo -e "${WBBG}       [ Senarai Login Akaun Vless ]        ${NC}";
echo -e "${CYAN}════════════════════════════════════════════${NC}";
echo -e "";
for akun in "${data[@]}"
do
...
if [[ "$jum" = "$ip" ]]; then
echo "$jum" >> /etc/kaizenvpn/cache/vless_temp.txt;
else
echo "$ip" >> /etc/kaizenvpn/cache/vless_temp2.txt;
fi
jum2=$(cat /etc/kaizenvpn/cache/vless_temp.txt);
sed -i "/$jum2/d" /etc/kaizenvpn/cache/vless_temp2.txt > /dev/null 2>&1;
done
jum=$(cat /etc/kaizenvpn/cache/vless_temp.txt);
if [[ -z "$jum" ]]; then
echo > /dev/null;
else
jum2=$(cat /etc/kaizenvpn/cache/vless_temp.txt | nl);
...
echo "=============================================";
echo -n > /etc/kaizenvpn/cache/vless_temp3.txt;
data=(`cat /etc/xray-mini/client.conf | grep '^Vless' | cut -d " " -f 2`);
for akun in "${data[@]}"
do
if [[ -z "$akun" ]]; then
...
echo -e "${CYAN}════════════════════════════════════════════${NC}";
echo -e "${WBBG}    [ Senarai Login Akaun GRPC Vless ]      ${NC}";
echo -e "${CYAN}════════════════════════════════════════════${NC}";
oth=$(cat /etc/kaizenvpn/cache/vless_temp2.txt | sort | uniq | nl );
echo "GRPC IP : All User IP";
echo "$oth";
echo -e "${CYAN}════════════════════════════════════════════${NC}";
echo "";
rm -rf /etc/kaizenvpn/cache/vless_temp2.txt;
rm -rf /etc/kaizenvpn/cache/vless_temp.txt;
rm -rf /etc/kaizenvpn/cache/vless_temp3.txt;
