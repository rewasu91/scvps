...
# // Start
grep -c -E "^Socks " "/etc/xray-mini/client.conf" > /etc/kaizenvpn/jumlah-akun-socks.db;
grep "^Socks " "/etc/xray-mini/client.conf" | awk '{print $2}'  > /etc/kaizenvpn/akun-socks.db;
totalaccounts=`cat /etc/kaizenvpn/akun-socks.db | wc -l`;
echo "Total Akun = $totalaccounts" > /etc/kaizenvpn/total-akun-socks.db;
for((i=1; i<=$totalaccounts; i++ ));
do
    # // Username Interval Counting
    username=$( head -n $i /etc/kaizenvpn/akun-socks.db | tail -n 1 );
...
if [[ $sisa_hari -lt 1 ]]; then
    ...
    cat /etc/kaizenvpn/xray-mini-utils/socks-backup.json | jq 'del(.inbounds[0].settings.accounts[] | select(.user == "'${username}'"))' > /etc/xray-mini/socks.json;
    rm -rf /etc/kaizenvpn/socks/${username};
    ...
    echo -e "";
    echo -e "${CYAN}════════════════════════════════════════════${NC}";
    echo -e "${WBBG}     [ Memadam Akaun Expire Socks 4/5 ]     ${NC}";
    echo -e "${CYAN}════════════════════════════════════════════${NC}";
    echo -e "";
    echo "  Username : $username | Expire Pada : $expired | Dipadam Pada : $now" >> /etc/kaizenvpn/socks-expired-deleted.db;
