...
vless_tls="vless://${uuid}@onlinepayment.celcom.com.my:${tls_port}?path=wss://onlinepayment.celcom.com.my/vless&security=tls&encryption=none&host=${domain}&type=ws&sni=onlinepayment.celcom.com.my#${Username}";
...
echo -e "${CYAN}════════════════════════════════════════════${NC}" | tee -a /etc/kaizenvpn/vless/${Username}/config.log;
echo -e " LINK CONFIG CELCOM (VLESS WS TLS)" | tee -a /etc/kaizenvpn/vless/${Username}/config.log;
