...
cat > /etc/kaizenvpn/cache/vmess-nontls.tmp << END
{"add":"162.159.134.61","aid":"0","host":"${domain}","id":"${uuid}","net":"ws","path":"/vmess","port":"${nontls_port}","ps":"${Username}","scy":"none","sni":"","tls":"","type":"","v":"2"}
END
...
echo -e "${CYAN}════════════════════════════════════════════${NC}" | tee -a /etc/kaizenvpn/vmess/${Username}/config.log;
echo -e " LINK CONFIG DIGI (VMESS WS NONE TLS)" | tee -a /etc/kaizenvpn/vmess/${Username}/config.log;
