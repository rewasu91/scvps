...
echo -e "${CYAN}══════════════════════════════════════════${NC}";
echo -e "";
echo "  $(date) Autokill Vmess Multilogin Versi 2.0 (Multiport).";
...
# // Start
# ════════
grep -c -E "^Vmess " "/etc/xray-mini/client.conf" > /etc/kaizenvpn/jumlah-akun-vmess.db;
grep "^Vmess " "/etc/xray-mini/client.conf" | awk '{print $2}'  > /etc/kaizenvpn/akun-vmess.db;
totalaccounts=`cat /etc/kaizenvpn/akun-vmess.db | wc -l`;
echo "Total Akun = $totalaccounts" > /etc/kaizenvpn/total-akun-vmess.db;
for((i=1; i<=$totalaccounts; i++ ));
do
    # // Username Interval Counting
    username=$( head -n $i /etc/kaizenvpn/akun-vmess.db | tail -n 1 );
    ...
    if [[ $jumlah_baris_log -gt $VMESS ]]; then
        ...
        cat /etc/kaizenvpn/xray-mini-utils/tls-backup.json | jq 'del(.inbounds[2].settings.clients[] | select(.email == "'${username}'"))' > /etc/kaizenvpn/xray-mini-cache.json;
        cat /etc/kaizenvpn/xray-mini-cache.json | jq 'del(.inbounds[5].settings.clients[] | select(.email == "'${username}'"))' > /etc/xray-mini/tls.json;
        cp /etc/xray-mini/nontls.json /etc/kaizenvpn/xray-mini-utils/nontls-backup.json;
        cat /etc/kaizenvpn/xray-mini-utils/nontls-backup.json | jq 'del(.inbounds[0].settings.clients[] | select(.email == "'${username}'"))' > /etc/xray-mini/nontls.json;
        rm -rf /etc/kaizenvpn/vmess/${username};
        ...
        echo "VMESS - $(date) - ${username} - Multi Login Detected - Killed at ${date}" >> /etc/kaizenvpn/autokill.log;
    fi
