...
# // Start
# ════════
grep -c -E "^Shadowsocks " "/etc/xray-mini/client.conf" > /etc/kaizenvpn/jumlah-akun-shadowsocks.db;
grep "^Shadowsocks " "/etc/xray-mini/client.conf" | awk '{print $2}'  > /etc/kaizenvpn/akun-shadowsocks.db;
totalaccounts=`cat /etc/kaizenvpn/akun-shadowsocks.db | wc -l`;
echo "Total Akun = $totalaccounts" > /etc/kaizenvpn/total-akun-trojan.db;
for((i=1; i<=$totalaccounts; i++ ));
do
    # // Username Interval Counting
    username=$( head -n $i /etc/kaizenvpn/akun-shadowsocks.db | tail -n 1 );
    ...
    jumlah_baris_log=$( cat /etc/kaizenvpn/xray-mini-shadowsocks/access.log | grep -w $hariini | grep -w $waktu | grep -w 'accepted' | grep -w $username | wc -l );

    if [[ $jumlah_baris_log -gt $SHADOWSOCKS ]]; then
        cp /etc/xray-mini/shadowsocks.json /etc/kaizenvpn/xray-mini-utils/shadowsocks-backup.json;
        cat /etc/kaizenvpn/xray-mini-utils/shadowsocks-backup.json | jq 'del(.inbounds[0].settings.clients[] | select(.email == "'${username}'"))' > /etc/xray-mini/shadowsocks.json;
        rm -rf /etc/kaizenvpn/shadowsocks/${username};
        ...
        echo "SHADOWSOCKS - $(date) - ${username} - Multi Login Detected - Killed at ${date}" >> /etc/kaizenvpn/autokill.log;
    fi
# // End Function
done
done
