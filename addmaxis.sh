...
#vless_tls="vless://${uuid}@who.int:${tls_port}?path=wss://who.int/vless&security=tls&encryption=none&host=${domain}&type=ws&sni=who.int#${Username}";
vless_tls="vless://${uuid}@172.67.9.227:${tls_port}?path=wss://italeem.iium.edu.my/vless&security=tls&encryption=none&host=${domain}&type=ws&sni=italeem.iium.edu.my#${Username}";
...
echo -e "${CYAN}════════════════════════════════════════════${NC}" | tee -a /etc/kaizenvpn/vless/${Username}/config.log;
echo -e " LINK CONFIG MAXIS (VLESS WS TLS)" | tee -a /etc/kaizenvpn/vless/${Username}/config.log;
