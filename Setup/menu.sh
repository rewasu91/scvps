...
function registerProvider() {
	...
	sed -i '/^EPG/d' /etc/kaizensystem/iptv/epg
	sed -i '/^$/d' /etc/kaizensystem/iptv/epg
	echo "EPG=${EPG}">>/etc/kaizensystem/iptv/epg
	echo -n -e "${GREEN}done${NOCOLOR}"
	sleep 1
	clear
	echo
...
function createIptvUser() {
	source /etc/kaizensystem/iptv/epg
	clear
	echo -e ""
	cowsay -f dragon "SELAMAT DATANG BOSKU."
	figlet -k   RKD OTT
...
1. Sila copy URL ini <b>${URL_SHORTENED}</b>
2. Masukkan URL yang telah dicopy kedalam apps OTT. <a href=\"https://youtu.be/asRn2XMkYck\"><b>CARA MASUKKAN URL</b></a>
3. Selepas dah masukkan URL dan apply, sila pm admin semula untuk verify device anda. PM admin, cakap Done Login
...
function gorestore() {
	...
	rm /etc/kaizensystem/iptv/epg
	EPG_URL=http://tny.im/pNjfW
	EPG=${EPG_URL}
	sed -i '/^EPG/d' /etc/kaizensystem/iptv/epg
	sed -i '/^$/d' /etc/kaizensystem/iptv/epg
	echo "EPG=${EPG}">>/etc/kaizensystem/iptv/epg
