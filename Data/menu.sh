...
TOKEN=$(sed -n '1p' /root/.bckupbot | awk '{print $1}')
ChatID=$(sed -n '2p' /root/.bckupbot | awk '{print $1}')
...
function registerProvider() {
	...
	until [[ ! -z ${PROVIDER_NAME} && $(grep -c -w "${PROVIDER_NAME}" /etc/silkroad/iptv/providerlist.txt) == 0 ]]; do
	...
	#curl -x socks5h://127.0.0.1:40000 -sL -o /etc/silkroad/iptv/${PROVIDER_NAME}.playlist.tmp "${LINK}" -A "OTT Navigator/1.6.8.3 (Linux;Android 11; en; ${OTT_HASH})"
	
	curl -x socks5h://127.0.0.1:40000 -sL -o /etc/silkroad/iptv/${PROVIDER_NAME}.playlist.tmp "${LINK}" -A "${OTT}"
	...
	cp -f /etc/silkroad/iptv/${PROVIDER_NAME}.playlist /etc/silkroad/iptv/silkroad.playlist
	#sed -i "/^${PROVIDER_NAME}/d" /etc/silkroad/iptv/providerlist.txt
	#sed -i '/^$/d' /etc/silkroad/iptv/providerlist.txt
	echo "${PROVIDER_NAME} ${LINK} ${OTT}">>/etc/silkroad/iptv/providerlist.txt
	sed -i '/^EPG/d' /etc/silkroad/parameter
	sed -i '/^$/d' /etc/silkroad/parameter
	echo "EPG=${EPG}">>/etc/silkroad/parameter
	echo -n -e "${GREEN}done${NOCOLOR}"
...
function createIptvUser() {
	...
	TODAY_IN_SECONDS=$(date +%s)
	USER_NUMBER=$(ls -w 1 /var/www/html/iptv | wc -l)
	#USER_NUMBER=$(ls -w 1 /etc/silkroad/wgcf | sed '/version.txt/d')
	#USER_NUMBER="$(awk -F: '$3 >= 1000 && $1 != "nobody" {print $1}' /etc/silkroad/iptv/userlist.txt | wc -l)";	
	cut -d ' ' -f2-3 /etc/silkroad/iptv/userlist.txt | cut -d '=' -f2-3 | sed 's/EXP_DATE=//' | sort >/etc/silkroad/iptv/userlist.tmp 
	...
	ChatID=$(sed -n '2p' /root/.bckupbot | awk '{print $1}')
	TEXT="<b>RKD OTT TV</b>
<a href=\"https://youtu.be/asRn2XMkYck\">RKD OTT TV</a>
